...
public class GroupedBlockingQueue<E> implements BlockingQueue<E> {
    ...
    public boolean offer(E element) {
    	String group = GroupedExecutor.GroupedFutureTask.class.equals(element.getClass()) && GroupedExecutor.TaskWrapper.class.equals(((GroupedExecutor.GroupedFutureTask) element).getTask().getClass()) ?
    			((GroupedExecutor.TaskWrapper) ((GroupedExecutor.GroupedFutureTask) element).getTask()).getGroup() : "";
        ...
        synchronized (this) {
	        ...
	        while (true) {
	            for (int i = 0; i < groupCount; i++) {
	                int currentIndex = (previousGroupIndex + 1) % groupCount; // Calculate the index of the next group
	
	                String group = groupKeys.get(currentIndex);
...
//		                	System.out.println("launching " + group);
	                	...
	                	System.err.println("tadam " + group);
	            }
                wait();
            }
        }
